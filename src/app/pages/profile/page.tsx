...
const ScaleIcon = (props: SVGProps<SVGSVGElement>) => ( 
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v17.25m0 0c-1.472 0-2.882.265-4.185.75M12 20.25c1.472 0 2.882.265 4.185.75M18.75 4.97A48.416 48.416 0 0 0 12 4.5c-2.291 0-4.545.16-6.75.47m13.5 0c1.01.143 2.01.317 3 .52m-3-.52M6.272 5.05A48.416 48.416 0 0 1 12 4.5c2.291 0 4.545.16 6.75.47m0 0c-.98.132-1.98.296-3 .479M12 9.75V14.25m0-4.5c0-1.483.673-2.857 1.757-3.753M12 9.75c-1.084 0-1.757.87-1.757 2.25S10.916 14.25 12 14.25m-8.25 4.5a4.5 4.5 0 0 1-1.887-8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887 8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887m11.25-8.456a4.5 4.5 0 0 1-1.887 8.456 4.5 4.5 0 0 1 8.456-1.887m0-1.887a4.5 4.5 0 0 1 1.887-8.456 4.5 4.5 0 0 1-8.456 1.887" />
  </svg>
);
...
// 2. Impor LoginButton secara dinamis dengan ssr: false
const LoginButton = dynamic(() => import('@/components/LoginButton'), {
  ssr: false,
  // Anda bisa menambahkan loading state jika diperlukan
...
export default function FinanceProfilePage() {
  ...
  let telegramUser: any = null; // Inisialisasi telegramUser dengan null atau tipe yang sesuai
  ...
  const [profileData, setProfileData] = useState({
    name: "Nama Pengguna", // Default name
    email: "", // Email tidak tersedia langsung dari initData standar
    memberSince: "", // Tanggal bergabung tidak tersedia langsung
    avatarUrl: "", 
    recentActivity: [
      // ... data aktivitas
    ],
  ...
  useEffect(() => {
    if (telegramUser) {
      setProfileData(prevData => ({
        ...prevData,
        name: `${telegramUser.firstName || ''} ${telegramUser.lastName || ''}`.trim() || telegramUser.username || 'Pengguna Telegram',
        avatarUrl: telegramUser.photoUrl || "",
        // Anda bisa menambahkan logika untuk email dan memberSince jika ada sumber data lain
        // email: telegramUser.email, // Contoh jika email ada
        // memberSince: // Logika untuk tanggal bergabung
      }));
    }
  }, [telegramUser]);

  // Menambahkan penanganan jika telegramUser belum termuat atau tidak ada
  if (!telegramUser) {
    return (
      <div className="min-h-screen flex items-center justify-center font-sans bg-background text-foreground">
        <p>Memuat data pengguna atau pastikan aplikasi dibuka melalui Telegram...</p>
      </div>
    );
  }
      ...
      <header className="py-6 sm:py-8 px-4 sm:px-6 bg-card shadow-md sticky top-0 z-20 border-b border-border">
        <div className="container mx-auto max-w-3xl">
          <div className="flex flex-col items-center text-center sm:flex-row sm:justify-between sm:text-left">
            ...
            <div className="flex space-x-2 mt-4 sm:mt-0 items-center"> {/* Ditambahkan items-center */}
              <LoginButton /> {/* Ditambahkan */}
...
// Hapus deklarasi ProfilePage duplikat di bawah ini
