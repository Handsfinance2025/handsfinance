...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("API Route (/api/llm/gemini/route.ts): Sending request to Gemini API with query:", userQuery);
    ...
    if (result.candidates && result.candidates.length > 0 &&
      ...
      console.warn('API Route (/api/llm/gemini/new_scraping/): Respons dari Gemini tidak memiliki format yang diharapkan atau tidak ada kandidat:', JSON.stringify(result, null, 2));
