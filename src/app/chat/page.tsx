  ...
  return (
    <div style={{ backgroundColor: colors.lightGray }} className="flex flex-col h-screen max-w-2xl mx-auto font-sans relative">
      ...
      <div className="flex-grow overflow-y-auto p-6 space-y-4">
        {messages.map((msg) => (
          ...
          >
            <div
              style={{
                backgroundColor: msg.sender === 'user' ? colors.primaryGreen : colors.ashGray,
                color: msg.sender === 'user' ? colors.lightGray : colors.darkGreenGray,
                boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1), 0 2px 6px rgba(0, 0, 0, 0.08)' // 3D shadow
              }}
              className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-3 rounded-xl ${
                msg.sender === 'user' ? 'rounded-br-none' : 'rounded-bl-none'
              } ${msg.isLoading ? 'animate-pulse opacity-80' : ''} transition-shadow duration-300 hover:shadow-xl`}
            >
      ...
      <form
        ...
        className="p-3 sm:p-4 border-t sticky bottom-0 z-20 shadow-[0_-2px_10px_rgba(0,0,0,0.03)]" // Subtle top shadow for 3D effect
      >
        <div className="flex items-center space-x-2">
          <input
            ...
            className={`flex-grow p-3 border rounded-lg focus:ring-2 
                            focus:ring-[${colors.primaryGreen}] 
                            focus:border-[${colors.primaryGreen}] 
                            outline-none transition-all duration-200 ease-in-out 
                            text-[${colors.darkGreenGray}] placeholder:text-gray-400 bg-white`}
            disabled={isLoading}
          />
          <button
            type="submit"
            style={{ backgroundColor: colors.primaryGreen, color: colors.lightGray }}
            className={`p-3 rounded-lg hover:opacity-90 focus:outline-none 
                            focus:ring-2 focus:ring-[${colors.primaryGreen}] focus:ring-offset-1 
